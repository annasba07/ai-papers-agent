      ...
      <div className="paper-card__header">
        <div className="paper-card__content">
          ...
          <div className="paper-card__badges">
            {/* Code availability - PRIORITY: Show first for reproducibility */}
            {hasCode() && !githubStats && (
              <span className="badge badge-success badge-code" title="Code repository available">
                💻 Code Available
              </span>
            )}
            {hasCode() && githubStats && (
              <a
                ...
                className="badge badge-success badge-code badge-link"
                onClick={(e) => e.stopPropagation()}
                title={`View code on GitHub - ${githubStats.total_stars.toLocaleString()} stars`}
              >
                💻 View Code ({formatStars(githubStats.total_stars)})
              </a>
            )}
            {githubStats && githubStats.total_stars > 0 && !hasCode() && (
            ...
            {/* Industry best practice: Two-tier citation badge system for newcomers */}
            {/* Seminal papers (500+ citations) - Must-read foundational work */}
            {paper.citation_count >= 500 && (
              <span className="badge badge-seminal" title={`${paper.citation_count.toLocaleString()} citations - Top 1% highly cited`}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                </svg>
                Seminal ({paper.citation_count >= 1000 ? `${(paper.citation_count / 1000).toFixed(1)}k` : paper.citation_count.toLocaleString()})
              </span>
            )}
            {/* Foundational papers (100-499 citations) - Important contextual work */}
            {paper.citation_count >= 100 && paper.citation_count < 500 && (
              <span className="badge badge-foundational" title={`${paper.citation_count.toLocaleString()} citations`}>
                ...
                Foundational ({paper.citation_count.toLocaleString()})
              </span>
            )}
            {/* Beginner-friendly paper type badges */}
            {paper.paper_type === "survey" && (
              <span className="badge badge-info" title="Comprehensive overview of a research area">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            {paper.paper_type === "tutorial" && (
              <span className="badge badge-success" title="Step-by-step educational content">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            {paper.paper_type === "review" && (
              <span className="badge badge-info" title="Critical review of research literature">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                  <polyline points="14 2 14 8 20 8"/>
            ...
            {paper.paper_type === "primer" && (
              <span className="badge badge-success" title="Introductory guide for newcomers">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
